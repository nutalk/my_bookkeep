...
async function createLiability(
  ...
  // 以流水为准：写入期初借款流水，保持余额与流水一致
  if (saved && saved.remainingPrincipal > 0) {
    await db.insert(transactions).values({
      userId,
      type: "liability_principal_change",
      liabilityId: insertId,
      amount: saved.remainingPrincipal,
      description: "初始金额",
      transactionDate: saved.startDate ?? new Date(),
...
async function updateAsset(
  ...
  // 以流水为准：手工改当前价值时补一笔价值变动流水
  const nextValue =
    currentValue !== undefined ? Number(currentValue) : before.currentValue;
  const delta = nextValue - before.currentValue;
