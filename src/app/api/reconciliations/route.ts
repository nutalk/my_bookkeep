...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: "获取对账记录失败" }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const reconciliationResult = await db.insert(reconciliations).values({
      userId: user.id,
    ...
    if (Math.abs(difference) > 0.001) {
      ...
      if (body.liabilityId) {
        ...
        await db
          .update(liabilities)
          .set({
            remainingPrincipal: body.actualBalance,
            updatedAt: new Date(),
          })
          .where(
            and(
              eq(liabilities.id, body.liabilityId),
    ...
    return NextResponse.json(
      { reconciliation, transaction: null },
      { status: 201 },
    );
  } catch (e) {
    if ((e as Error).message === "UNAUTHORIZED") {
    ...
    return NextResponse.json({ error: "创建对账记录失败" }, { status: 500 });
  }
}
